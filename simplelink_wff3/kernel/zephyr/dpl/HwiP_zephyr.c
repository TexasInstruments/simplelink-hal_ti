...
#define INT_CPUIRQ1 -1
#define INT_CPUIRQ3 -2
#define INT_CPUIRQ4 -3
#define INT_CPUIRQ16 -4
#define INT_CPUIRQ0 -5
#define INT_LRFD_IRQ0 -6
#define INT_LRFD_IRQ1 -7
...
static struct sl_isr_args sl_UART0_IRQ_cb = {NULL, 0};
...
static unsigned int sl_irq_priority(uint32_t priority)
{
...
HwiP_Handle HwiP_construct(HwiP_Struct *handle, int interruptNum, HwiP_Fxn hwiFxn, HwiP_Params *params)
{
    ...
    priority = sl_irq_priority(priority);
    ...
    switch (interruptNum)
    {
        ...
        case INT_LRFD_IRQ1:
            ...
            break;
        case INT_SP_UART_0_INT_REQ:
            sl_UART0_IRQ_cb.cb  = hwiFxn;
            sl_UART0_IRQ_cb.arg = arg;
            obj->cb             = &sl_UART0_IRQ_cb;
            irq_connect_dynamic(INT_SP_UART_0_INT_REQ - 16, priority, sl_isr, &sl_UART0_IRQ_cb, 0);
            break;
        case INT_SP_UART_1_INT_REQ:
            sl_UART1_IRQ_cb.cb  = hwiFxn;
    ...
    if (params == NULL || params->enableInt)
    {
...
{
    if (interruptNum == INT_PENDSV)
    {
        SCB->ICSR |= SCB_ICSR_PENDSVCLR_Msk;
    }
    else if (interruptNum == INT_SYSTICK)
    {
        SCB->ICSR |= SCB_ICSR_PENDSTCLR_Msk;
    }
    else if (interruptNum >= 16)
    {
        NVIC_ClearPendingIRQ(interruptNum - 16);
    }
}
...
{
    if (interruptNum == INT_NMI_FAULT)
    {
        SCB->ICSR |= SCB_ICSR_NMIPENDSET_Msk;
    }
    else if (interruptNum == INT_PENDSV)
    {
    ...
    {
        NVIC_SetPendingIRQ(interruptNum - 16);
    }
}
...
void HwiP_setPriority(int interruptNum, uint32_t priority)
{
    ...
    if (interruptNum >= 16)
    {
        z_arm_irq_priority_set(interruptNum - 16, sl_irq_priority(priority), 0);
    }
