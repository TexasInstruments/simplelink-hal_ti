...
#define NVOCMP_RW_BLOCK_SIZE 16
...
static uint8_t NVOCMP_initNvApi(void *param)
{
    ...
    if(NVOCMP_failF == NVINTF_NOTREADY)
    {
        ...
        // Parameters are not needed for CC35XX
        NVOCMP_nvsHandle = (XMEM_Handle)NVS_open(NVOCMP_NVS_INDEX);
...
#ifdef ENABLE_SPS
    if ((len % NVOCMP_RW_BLOCK_SIZE) > 0)
    {
        iLen  += (NVOCMP_RW_BLOCK_SIZE - (len % NVOCMP_RW_BLOCK_SIZE));
    }
#endif
                              ...
                              uint8_t *pBuf, NVOCMP_writeMode_t wm)
{
    ...
    if ((iHdr->len % NVOCMP_RW_BLOCK_SIZE) > 0)
    {
        iLen += (NVOCMP_RW_BLOCK_SIZE - (iHdr->len % NVOCMP_RW_BLOCK_SIZE));
    }
#endif
...
{
#ifndef NV_LINUX
  /* CC35XX has below 2 constraints. Variables to handle them.
   * - Constraint of reading multiple of 16-bytes at a time.
   * - Constraint of reading from 4W aligned address.
   */
  uint8_t dataRead[NVOCMP_RW_BLOCK_SIZE] __attribute__((aligned(4)));
  uint8_t numReads = 0;
  uint16_t offsetAligned = off;
  uint16_t lenRemaining = len;
  uint16_t bytesRead = 0;

  // Handling the constraint of reading from 4W aligned address.
  uint8_t offsetShift = 0;
  offsetShift = off % 4;
  if (offsetShift > 0)
  {
      offsetAligned = off - offsetShift;
  }

  // Handle the constraint of reading multiples of 16Bytes at a time.
  while (lenRemaining >= NVOCMP_RW_BLOCK_SIZE)
  {
      if (lenRemaining == len)
      {
          NVS_read(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(pg, offsetAligned), (uint8_t *)dataRead, NVOCMP_RW_BLOCK_SIZE);
          memcpy(pBuf, (dataRead + offsetShift), (NVOCMP_RW_BLOCK_SIZE - offsetShift));
          bytesRead = NVOCMP_RW_BLOCK_SIZE - offsetShift;
          // Setting to 0 for the last set of bytes less than 16, if any.
          offsetShift = 0;
      }
      else
      {
          NVS_read(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(pg, (offsetAligned + 16*numReads)), (uint8_t *)dataRead, NVOCMP_RW_BLOCK_SIZE);
          memcpy(pBuf+bytesRead, dataRead, NVOCMP_RW_BLOCK_SIZE);
          bytesRead += NVOCMP_RW_BLOCK_SIZE;
      }

      numReads += 1;
      lenRemaining = len - bytesRead;
  }

  /* Less than 16-bytes to read:
  * - Either after the loop for remaining bytes
  * - Or for first time read with less than 16-bytes length
  */
  if (lenRemaining > 0)
  {
      NVS_read(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(pg, (offsetAligned + 16*numReads)), (uint8_t *)dataRead, NVOCMP_RW_BLOCK_SIZE);
      memcpy((pBuf+bytesRead), (dataRead + offsetShift), lenRemaining);
  }
#else
...
static uint8_t NVOCMP_write(uint8_t dstPg, uint16_t off, uint8_t *pBuf, uint16_t len)
{
    ...
    {
#ifndef NV_LINUX
        /* CC35XX has below 2 constraints.
         * - Constraint of writing multiple of 16-bytes at a time.
         * - Constraint of writing to 4W aligned address/offset.
         */
        uint8_t dataToWrite[NVOCMP_RW_BLOCK_SIZE] __attribute__((aligned(4)));
        uint8_t numWrites = 0;
        uint16_t offsetAligned = off;
        uint16_t lenRemaining = len;
        uint16_t bytesWritten = 0;

        // CC35XX has a constraint of writing minimum of 16-bytes at a time.
        uint8_t offsetShift = 0;
        offsetShift = off % 4;
        if (offsetShift > 0)
        {
        ...
        // Handle the constraint of writing multiples of 16Bytes at a time.
        while (lenRemaining >= NVOCMP_RW_BLOCK_SIZE)
        {
            if (lenRemaining == len)
            {
                NVOCMP_read(dstPg, offsetAligned, (uint8_t *)dataToWrite, NVOCMP_RW_BLOCK_SIZE);
                memcpy((dataToWrite + offsetShift), pBuf, (NVOCMP_RW_BLOCK_SIZE - offsetShift));
                nvsRes = NVS_write(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(dstPg, offsetAligned),
                                  dataToWrite, NVOCMP_RW_BLOCK_SIZE, NVS_WRITE_POST_VERIFY);
                bytesWritten = NVOCMP_RW_BLOCK_SIZE - offsetShift;
                // Setting to 0 for the last set of bytes less than 16, if any.
                offsetShift = 0;
            }
            else
            {
                NVOCMP_read(dstPg, (offsetAligned + 16*numWrites), (uint8_t *)dataToWrite, NVOCMP_RW_BLOCK_SIZE);
                memcpy(dataToWrite, pBuf+bytesWritten, NVOCMP_RW_BLOCK_SIZE);
                nvsRes = NVS_write(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(dstPg, (offsetAligned + 16*numWrites)),
                                  dataToWrite, NVOCMP_RW_BLOCK_SIZE, NVS_WRITE_POST_VERIFY);
                bytesWritten += NVOCMP_RW_BLOCK_SIZE;
            }

            // Update write status so far.
            numWrites += 1;
            lenRemaining = len - bytesWritten;
        }

        /* Less than 16-bytes to write:
        * - Either after the loop for remaining bytes
        * - Or for first time write with less than 16-bytes length
        */
        if (lenRemaining > 0)
        {
            NVOCMP_read(dstPg, (offsetAligned + 16*numWrites), dataToWrite, NVOCMP_RW_BLOCK_SIZE);
            memcpy((dataToWrite + offsetShift), (pBuf + bytesWritten), lenRemaining);
            nvsRes = NVS_write(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(dstPg, (offsetAligned + 16*numWrites)),
                              dataToWrite, NVOCMP_RW_BLOCK_SIZE, NVS_WRITE_POST_VERIFY);
        }
#else
                             ...
                             uint8_t dstPg, uint16_t dstOff, uint8_t *pBuf)
{
...
#ifdef ENABLE_SPS
    if ((pHdr->len % NVOCMP_RW_BLOCK_SIZE) > 0)
    {
        iLen += (NVOCMP_RW_BLOCK_SIZE - (pHdr->len % NVOCMP_RW_BLOCK_SIZE));
    }
#endif
    ...
    if((dstOff + iLen) <= FLASH_PAGE_SIZE)
    {
        ...
        if ((pHdr->len % NVOCMP_RW_BLOCK_SIZE) > 0)
        {
            dLen = pHdr->len + (NVOCMP_RW_BLOCK_SIZE - (pHdr->len % NVOCMP_RW_BLOCK_SIZE));
            hOfs = dstOff + dLen;
        }
                               ...
                               void *pBuf, bool flag)
{
  ...
  uint16_t lenEncrypted;
    ...
    if ((len % NVOCMP_RW_BLOCK_SIZE) > 0)
    {
        iOfs = (iHdr->hofs - (iHdr->len + (NVOCMP_RW_BLOCK_SIZE - iHdr->len % NVOCMP_RW_BLOCK_SIZE)));
    }
#endif
        ...
        if((dOfs + len) <= iHdr->hofs)
        {
            ...
            else
            {
              ...
              // Update len for the read since encryption added padding data.
                if ((len % NVOCMP_RW_BLOCK_SIZE) > 0)
                {
                    lenEncrypted = len + (NVOCMP_RW_BLOCK_SIZE - (len % NVOCMP_RW_BLOCK_SIZE));
                }
                else
                {
                    lenEncrypted = len;
      ...
      while(ofs >= (NVOCMP_PGDATAOFS + NVOCMP_ITEMHDRLEN))
      {
          ...
          if(iHdr.stats & NVOCMP_FOLLOWBIT)
          {
              ...
              if(iHdr.len < ofs)
              {
                  ...
                  if ((iHdr.len % NVOCMP_RW_BLOCK_SIZE) > 0)
                  {
                      ofs -= (NVOCMP_RW_BLOCK_SIZE - (iHdr.len % NVOCMP_RW_BLOCK_SIZE));
                  }
#endif
              }
              else
...
static NVOCMP_compactStatus_t NVOCMP_compact(NVOCMP_nvHandle_t *pNvHandle)
{
    ...
    while(srcPg != dstPg)
    {
        ...
        if(NVOCMP_failW == NVINTF_SUCCESS)
        {
            ...
            if ((dataLen % NVOCMP_RW_BLOCK_SIZE) > 0)
            {
                itemSize += (NVOCMP_RW_BLOCK_SIZE - (dataLen % NVOCMP_RW_BLOCK_SIZE));
                crcOff   -= (NVOCMP_RW_BLOCK_SIZE - (dataLen % NVOCMP_RW_BLOCK_SIZE));
            }
#endif
            ...
            else
            {
              ...
              if ((dataLen % NVOCMP_RW_BLOCK_SIZE) > 0)
              {
                  dataLen += (NVOCMP_RW_BLOCK_SIZE - (dataLen % NVOCMP_RW_BLOCK_SIZE));
              }
#endif
              srcOff -= dataLen;
            }
...
static uint8_t NVOCMP_verifyCRC(uint16_t iOfs, uint16_t len, uint8_t crc, uint8_t pg, bool flag)
{
    ...
    if ((len % NVOCMP_RW_BLOCK_SIZE) > 0)
    {
        crcLen += (NVOCMP_RW_BLOCK_SIZE - (len % NVOCMP_RW_BLOCK_SIZE));
    }
#endif
