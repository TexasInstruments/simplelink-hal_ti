...
typedef struct k_thread EM_thread_type;

/* Datatype to represent Thread or Task Attribute */
typedef struct _EM_thread_attr_type {CHAR * name; void * stack_ptr; UINT16 stack_size; UINT8 prio;} EM_thread_attr_type;

/* Datatype to represent Mutex object */
typedef struct k_mutex EM_thread_mutex_type;

/* Datatype to represent Attributes of a Mutex object */
typedef struct _EM_thread_mutex_attr_type {void * dummy;} EM_thread_mutex_attr_type;

/* Datatype to represent Conditional Variable object */
/* typedef struct k_sem EM_thread_cond_type;*/
typedef struct k_condvar EM_thread_cond_type;

/* Datatype to represent Attributes of a Conditional Variable object */
typedef struct _EM_thread_cond_attr_type {void * dummy;} EM_thread_cond_attr_type;

/* Datatype to represent the OS time for the platform */
typedef INT64 EM_time_type;

/* Data types for task parameters and retun types */
typedef void EM_THREAD_RETURN_TYPE;
typedef void* EM_THREAD_ARGS;
typedef EM_THREAD_RETURN_TYPE (*EM_THREAD_START_ROUTINE)(EM_THREAD_ARGS, EM_THREAD_ARGS, EM_THREAD_ARGS);
...
/* --------------------------------------------------- Macros */
/* OS Stack macros */
#define EM_THREAD_STACK_DEFINE(ptr, size)        K_THREAD_STACK_DEFINE(ptr, size)
#define EM_THREAD_STACK_SIZE(ptr)                K_THREAD_STACK_SIZEOF(ptr)

/* Mutex macros */
#define EM_MUTEX_DEFINE(name)                    K_MUTEX_DEFINE(name)

/* CondVar macros */
#define EM_COND_DEFINE(name)                     K_CONDVAR_DEFINE(name)
 ...
 */
#define EM_alloc_mem_ext(s)           PL_assign_mem((size_t)(s))
#define EM_free_mem_ext(ptr)

/** Macro to find Minimum value between 2 given entities */
#define EM_GET_MIN(a, b) \
        (((a) > (b)) ? (b) : (a))

/** Macro to find Maximum value between 2 given entities */
#define EM_GET_MAX(a, b) \
        (((a) > (b)) ? (a) : (b))
